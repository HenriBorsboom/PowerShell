...
Write-Color "1. ", "Checking for supplied extensions" -ForegroundColor DarkCyan, White
...
ForEach ($Profile in $Profiles) {
    ...
    ForEach ($Extension in $Extensions) {
        Try {
            Write-Color -Text "|- ", "Profiles: ", $ProfilesCount, "/", $Profiles.Count, " - Processing Extension: ", $Extension.ToUpper(), " - " -ForegroundColor DarkCyan, White, Cyan, White, Cyan, White, Cyan, White -NoNewLine
            ...
            If ($TempFiles -ne $null) {
                ForEach ($TempFile in $TempFiles) {
                    ...
                    Try   {
                        If ($Force) { $empty = Remove-Item $TempFileFullPath -Recurse -Force -ErrorAction Stop } Else { $empty = Remove-Item $TempFileFullPath -Recurse -ErrorAction Stop }
                        $FilesRemoved ++
    ...
    Write-Color -Text "|- ", "Profiles: ", $ProfilesCount, "/", $Profiles.Count, " - Temporary Folder: ", $TempFolderFullPath.ToUpper(), " - " -ForegroundColor DarkCyan, White, Cyan, White, Cyan, White, Cyan, White -NoNewLine
    Try {
