<#
Release notes
!! From version 4, version notes will now be at the top and previous notes will be below it (Z - A instead of A - Z).
   all old notes are in the same sequence at the bottom.

 Version 4.0
 Information gathering processes now generate a CSV file
 The generated CSV files are then placed in a ZIP file
 This ZIP file now replaces the reporting attachement in the email
 (The ZIP file is then used by System Center Orchestrator to upload to a SQL DB)
 New Functions:
 * Generate-CSV
   - This is to create the CSV
 * New-Zip
   - This is to compress the CSV files and remove CSV files
 Changed Layout of $VMCriticalExclusionList
 Changed the $ReportingEnvironment variable to a Global variable
 
 version 3.3
 Redone Get- Functions to combine Hyper-V and VMWare
 Added Formatting function to Get-Datastores to format sizes to B/MB/GB/TB
 Corrected Get-Datastores function for VMWare to change MB to B instead of KB
 Reworked the Send-Report function to test if port 587 or port 25 is open and send with the open port
 Removed the PORT Parameter from the Send-Report function
 Added VMCriticalExclusionList to mark VMs as Warning when they have flags set to the name
 Updated the Get-VM function to not flag systems with an exclusion flag in the name as Critical but as Warning
 Updated the Process-vCenterVMs function to not flag systems with an exclusion flag in the name as Critical but as Warning
 Updated Get-Datastore Function to use Thresholds instead of hard coded
 Updated Process-Datastore Function to use Thresholds instead of hard coded
 Updated HTML processing functions to add NULL values
 Updated Footer details of report to inclue Script Execution Details
 Updated all PSOBjects so that full Column Name displays. (FreePerc became 'Free %')
 Removed the ValidateSet from Get-* Functions
 Added SilentContinue if no events found matching Hyper-V Events
 Corrected the Get-Hosts function for Hyper-V for Power State to reflect PoweredOn/PoweredOff
 Version 3.4.1
 Corrected the Script Version variable to String instead of floating point
 Updated Get-VM function for Hyper-V to query WMI for IP of the VM on older systems and use Hyper-V module on newer
 Corrected Get-VMs for Hyper-V to flag Offline VMs properly
 Corrected the Post-ScriptExecution section with titles
 Version 3.4.2
 Updated VMCriticalExclusionList
 Version 4.5.1
 Added VMWare Tools Running State to VMs
 Default of Old Reports changed from 8 days to 1 day
#> # Notes
Param (
...
#region Variables
# Alarms
$AlarmDateRangeInDays     = 1                                    # How many days back must it look for alarms - Only effective on Hyper-V
# Datastores
$WarningRange             = 20                                   # Anything between the Critical Range and the Warning Range are Warning
$CriticalRange            = 10                                   # Anything from the Critical Range and below are Critical
# VMs
$VMCriticalExclusionList  = @(`
    '*do not power*', `
    '*_clone*', `
    '*leave offline*',`
    '*do not delete*',`
    ...
    '*template*',`
    '*[DR]*',`
    '*not power on*')                               # This is to tag VMs with special names as Warning instead of Critical
# VM Snapshots
$AllowedSnapshotAge       = 7                                    # How old must a snapshot be before it is flagged as Critical
# E-Mail Setup                                                   # No need to specify SMTP port. Port 587 and 25 will be tested and sent with open port
$SMTPServer               = 'za-smtp-outbound-1.mimecast.co.za'  
$To                       = 'mscloud@eoh.com'
$From                     = 'eohrt_vm_storage@eoh.com'
$MailPassword             = 'v3Rystr0nGP@ssword2019'
# Old Report Days
$OldReportDays            = 1                                    # How old must the reports be before being archived
# Global Variables
#Images
...
$Global:VMRunningIcon     = $null
#endregion

#region Connection
If ($IPAddress -eq '') {
    $Global:ReportingEnvironment = New-Object -TypeName PSObject -Property @{
        'System Name' = 'IMSSD';
...
Function Send-Report {
    ...
    [String] $Body = Get-Content $ReportFile
    ...
    Send-MailMessage -From $From -BodyAsHtml -Body $Body -SmtpServer $SMTPServer -Subject $Subject -To $To -Port $SMTPPort -Attachments $ZipFile -Credential $Credential
}
#endregion
...
Function Generate-CSV {
    Param (
        ...
        If (!(Test-Path 'C:\EOH_RT\CSV')) {
            New-Item -Path 'C:\EOH_RT\CSV' -ItemType Directory | Out-Null
        }

        $ReportDate = (Get-Date -Format 'dd-MM-yyyy')
        $FileName = ("C:\EOH_RT\CSV\" + $ReportingEnvironment.'Common Name' + " - " + $ReportDate + "_" + $Source + ".csv")
        If (Test-Path $FileName) { Remove-Item $FileName -Force }
        $Data | Select $ColumnSequence | Export-Csv -Path $Filename -Force -NoClobber -Encoding ASCII -Delimiter ";" -NoTypeInformation
}
Function New-Zip {
    ...
    $ReportDate = (Get-Date -Format 'dd-MM-yyyy')    
    $FileName = ("C:\EOH_RT\" + $ReportingEnvironment.'Common Name' + " - " + $ReportDate + ".zip")
    If (Test-Path -LiteralPath $FileName) { Remove-Item -LiteralPath $FileName -Force }
    [System.IO.Compression.ZipFile]::CreateFromDirectory('C:\EOH_RT\CSV', $FileName, $CompressionLevel, $false)
    ForEach ($File in (Get-ChildItem -Path 'C:\EOH_RT\CSV')) {
...
Function Get-Alarms {
    ...
    Switch ($Platform) {
        'HyperV' {
            ...
            [Object[]] $ActiveAlarms = $AllAlarms | Where-Object {$_.Message -like "*hyper*" -or $_.Message -like "*cluster*"}
    ...
    If ($ReportAlarms.Count -gt 0) { `
        ...
        Generate-CSV -ReportingEnvironment $ReportingEnvironment -Source 'Alarms' -Data $ReportAlarms -ColumnSequence @('Source', 'Event', 'Category', 'Time', 'Health')
    }
    Else { 
        $Global:AlarmImage = $NonCriticalImage48
        $Global:AlarmIcon = "[NonCriticalImage]"
        Generate-CSV -ReportingEnvironment $ReportingEnvironment -Source 'Alarms' -Data $null -ColumnSequence @('Source', 'Event', 'Category', 'Time', 'Health')
    }
...
Function Get-Hosts {
    ...
    Generate-CSV -ReportingEnvironment $ReportingEnvironment -Data $ReportHosts -ColumnSequence @('Name', 'State', 'Power State', 'Health') -Source 'Hosts'
...
Function Get-VMSRunning {
    ...
    Generate-CSV -ReportingEnvironment $ReportingEnvironment -Data $ReportVMs -ColumnSequence @('Name', 'IP Address', 'State', 'Tools State', 'Health') -Source 'VMs'
...
Function Get-SnapShots {
    ...
    Switch ($Platform) {
        ...
        'VMWare' {
            [Object[]] $Snapshots = Get-VM | Get-Snapshot | Select-Object VM, Name, Created
    ...
    If ($ReportSnapshots.Count -eq 0) {
        ...
        Generate-CSV -ReportingEnvironment $ReportingEnvironment -Data $null -ColumnSequence @('VM', 'Name', 'Creation Time', 'Age', 'Health') -Source 'Snapshots'
    }
    ElseIf ($ReportSnapshots.Count -gt 0) {
        $Global:SnapshotImage = $WarningImage48
        $Global:SnapshotIcon = "[WarningImage]"
        Generate-CSV -ReportingEnvironment $ReportingEnvironment -Data $ReportSnapshots -ColumnSequence @('VM', 'Name', 'Creation Time', 'Age', 'Health') -Source 'Snapshots'
    }
...
Function Get-Datastores {
    ...
    Generate-CSV -ReportingEnvironment $ReportingEnvironment -Data $ReportDatastores -ColumnSequence @('Name', 'Free Space', 'Capacity', 'Free %', 'Health') -Source 'Datastores'
...
If ($ServerConnect -ne $False) {
    ...
    $ZipFile = New-Zip
    Send-Report -Client $ReportingEnvironment.'Common Name' -ReportFile $ReportFile -ZipFile $ZipFile
