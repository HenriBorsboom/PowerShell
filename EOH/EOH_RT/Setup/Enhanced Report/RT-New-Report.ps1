...
Param (
    ...
    [Parameter(
                ...
                [String] $ReportFileNamePrefix = ("HyperVReport Tanzania Cluster" + "{0:D}" -f (Get-Date)).ToString(),
    [Parameter(
                Mandatory=$false,
    ...
    [Parameter(
                ...
                [String] $SMTPServer = ("smtp.sdcoe.net").ToString(),
    [Parameter(
                Mandatory=$false,
                HelpMessage='SMTP Server port number (Default 25)')]
                [Int16] $SMTPPort = "25".ToInt16(),
    [Parameter(
                Mandatory=$false,
                HelpMessage='Recipient e-mail address')]
                [Array] $MailTo = ("francisco.tamayo@sdcoe.net").ToString() ,
    [Parameter(
                Mandatory=$false,
                HelpMessage='Sender e-mail address')]
                [String] $MailFrom = ("DCadmin@sdcoe.net").ToString(),
    [Parameter(
                Mandatory=$false,
...
Function sGet-Wmi {
    ...
    # Base string
    $wmiCommand = "Get-WmiObject -ComputerName $ComputerName -Namespace $Namespace -Class $Class -ErrorAction Stop"

    # If available, add Filter parameter
    If ($Filter) {
        # $Filter = ($Filter -join ',').ToString()
        $Filter = [Char]34 + $Filter + [char]34               # [Char]34 = "
...
Function sConvert-Size {
    ...
    Elseif ($DiskVolumeSpaceUnit -eq "kb") {                                    # kilobyte input
...
Function sConvert-DiskPartitionStyle {
    ...
    If      ($PartitionStyleValue -eq 1)  { $Result = "MBR" }
    ElseIf  ($PartitionStyleValue -eq 2)  { $Result = "GPT" }
    Else                                  { $Result = "Unknown"}
    Return $Result
}
...
Function sConvert-VolumeSizeColors {
    ...
    If      (($FreePercent -le 10) -and ($FreePercent -gt 5)) { $Result = $stateBgColors[4],$stateBgColors[4],$stateWordColors[4]}
    ElseIf  ($FreePercent -le 5)                              { $Result = $stateBgColors[3],$stateBgColors[3],$stateWordColors[3]}
    Else                                                      { $Result = $stateBgColors[0],$stateBgColors[0],"#BDBDBD"}
    Return $Result
}
