...
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-ERMHYPV1';
    IPAddress  = '172.20.10.3'; 
    CommonName = 'Eskort - East Rand Mall';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-HBGHYPV1';
    IPAddress  = '172.20.16.2'; 
    CommonName = 'Eskort - Heidelberg 1';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-HBGHYPV2';
    IPAddress  = '172.20.16.55'; 
    CommonName = 'Eskort - Heidelberg 2';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-NSPHYPV1';
    IPAddress  = '172.20.13.6'; 
    CommonName = 'Eskort - Nelspruit';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-ROOHYPV1';
    IPAddress  = '172.20.11.61'; 
    CommonName = 'Eskort - Roodepoort';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-SVTHYPV1';
    IPAddress  = '172.20.20.6'; 
    CommonName = 'Eskort - Silverton';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-XVBHYPV1';
    IPAddress  = '172.20.9.3'; 
    CommonName = 'Eskort - Xavier';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKRT-EST-HYPV1';
    IPAddress  = '172.20.36.32'; 
    CommonName = 'Eskort - Escourt 1';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
$Sites += (New-Object -TypeName PSObject -Property @{ 
    SystemName = 'ESKORT-ESTHYPV2';
    IPAddress  = '172.20.36.230'; 
    CommonName = 'Eskort - Escourt 2';
    Platform   = 'HyperVStandalone'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    Username   = '';
    Password   = '';
})
...
Function Get-HyperVVMS {
    ...
    $ReportVMs = @()
    If ($VMHost -ne '') {
        $VMs = (Get-VM -ComputerName $VMhost | Select Name, State)
        ForEach ($VM in $VMs) {
...
If ($Sites.Count -eq 0) {
    Try {
        If ($ReportingEnvironment.Platform -eq 'VMWare') {
            Connect-VIServer -Server $ReportingEnvironment.IPAddress -Credential $Credentials -ErrorAction Stop | Out-Null
            $ServerConnect = $True
        }
        Else {
...
If ($ServerConnect -ne $False) {
    ...
    ElseIf ($ReportingEnvironment.Platform -eq 'HyperVStandalone') {
        ...
        $ReportVMs        = @()
        $ReportSnapshots  = @()
        ForEach ($Site in $ReportingEnvironment) {
            If (Test-Connection -ComputerName $Site.IpAddress -Count 1 -Quiet) {
                $ReportAlarms     += ,(Get-HyperVAlarms  -VMHost $Site.SystemName)
                $ReportHosts      += ,(Get-HyperVHosts -VMHost $Site.SystemName)
                $ReportVMs        += ,(Get-HyperVVMS -VMHost $Site.SystemName)
                $ReportSnapshots  += ,(Get-HyperVSnapShots -VMHost $Site.SystemName)
                $ReportDatastores += ,(Get-HyperVDatastores -VMHost $Site.SystemName)
            }
        }
    }
