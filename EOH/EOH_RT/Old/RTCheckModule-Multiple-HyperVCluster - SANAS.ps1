...
Function Load-HyperVHosts {
    $HyperVHosts = @()
    $HyperVHosts += ,(New-Object -TypeName PSObject -Property @{ IPAddress = '10.15.1.25'  ; CommonName = 'SANAS Cluster'})
...
Function Get-HyperVAlarms {
    ...
    $ActiveAlarms = $AllAlarms | Where-Object {$_.Message -like "*hyper*" -or $_.Message -like "*failover*"}
    $ReportAlarms = @()
...
Function Get-HyperVHosts {
    ...
    $HyperVHost = Get-VMHost -ComputerName $VMHost | Select Name
    $ReportHosts = @()
...
Function Get-ClusterDatastores {
    Param (
        [Parameter(Mandatory=$True, Position=1)]
        [String] $Cluster)
...
If ($ServerConnect -ne $False) {
    ...
    $ReportVMs        = @()
    $ReportSnapshots  = @()
    ForEach ($ClusterHost in (Get-ClusterNode -Cluster $HyperVHost.IPAddress)) { 
        ForEach ($ReturnAlarm in (Get-HyperVAlarms -VMHost $ClusterHost.Name)) {
            $ReportAlarms += $ReturnAlarm
        }
        ForEach ($ReturnHost in (Get-HyperVHosts -VMHost $ClusterHost.Name)) {
            $ReportHosts += $ReturnHost
        }
        ForEach ($ReturnVM in (Get-HyperVVMs -VMHost $ClusterHost.Name)) {
            $ReportVMs += $ReturnVM
        }
        ForEach ($ReturnSnapshot in (Get-HyperVSnapShots -VMHost $ClusterHost.Name)) {
            $ReportSnapshots += $ReturnSnapshot
        }
    }
    $ReportDatastores = Get-ClusterDatastores -Cluster $HyperVHost.IPAddress
}
Else {
