...
Function SendMail {
    ...
    Send-MailMessage -From $From -BodyAsHtml -Body $Body -SmtpServer $SMTPServer -Subject $Subject -To $To -Attachments $File -Credential $Credential #-UseSsl
}
Function Load-HyperVHosts {
    $HyperVHosts = @()
    $HyperVHosts += ,(New-Object -TypeName PSObject -Property @{ IPAddress = '192.168.1.21'  ; CommonName = 'Sekela Xabiso'})
...
Function Get-HyperVAlarms {
    $AllAlarms = Get-WinEvent -FilterHashtable @{LogName='System';StartTime=(Get-Date).AddDays(-1)}
    ...
    If ($ReportAlarms.Count -gt 1) { $Global:AlarmImage = $CriticalImage }
...
Function Process-vCenterVMs {
    ...
    If ($ReportVMs.count -gt 0) {
        ...
        for ($VMsHTMLIndex = 1; $VMsHTMLIndex -le $VMsHTML.table.tr.count - 1; $VMsHTMLIndex++) {
            if ($VMsHTML.table.tr[$VMsHTMLIndex].td[1] -ne 'Healthy') {
...
Write-Color  -Text "Collecting data for ", $HyperVHosts.CommonName, " - " -ForegroundColor White, Yellow, White -NoNewLine
#region Collect Data
Try {
    Test-Connection -ComputerName $HyperVHosts.IPAddress -Count 1 -Quiet | Out-Null
    $ServerConnect = $True
}
Catch {
    $ServerConnect = $False
...
If ($ServerConnect -ne $False) {
    $ReportAlarms     = Get-HyperVAlarms -VMHost $HyperVHosts.IPAddress
    $ReportHosts      = Get-HyperVHosts -VMHost $HyperVHosts.IPAddress
    $ReportVMs        = Get-HyperVVMs
    $ReportSnapshots  = Get-HyperVSnapShots
    $ReportDatastores = Get-HyperVDataStores -VMHost $HyperVHosts.IPAddress
}
Else {
...
$OutFile = ("C:\EOH_RT\" + $HyperVHosts.CommonName + " - " + $ReportDate + ".html")
Write-Color -Text "Generating HTML to ", $OutFile, " - " -ForegroundColor White, Yellow, White -NoNewLine
...
ConvertTo-Html @convertParams | out-file $OutFile -Force
SendMail -Client $HyperVHosts.CommonName -File $OutFile
Write-Color -Complete
#endregion
