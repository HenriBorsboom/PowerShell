Param (
    ...
    [String] $PasswordFile)
#region Connection
If ($SystemName -eq '' -and $IPAddress -eq '' -and $CommonName -eq '' -and $Platform -eq '') {
    $ReportingEnvironment = New-Object -TypeName PSObject -Property @{ 
        SystemName = 'EOHMIDRAND';
        IPAddress  = '10.1.5.254'; 
        CommonName = 'EOH Midrand Waterfall';
        Platform   = 'VMWare'; # Valid Options are HyperVCluster, HyperVStandalone, VMWare
    }
}
Else {
    $ReportingEnvironment = New-Object -TypeName PSObject -Property @{ 
...
Function Setup-Credentials {
    ...
    If ($File -ne '') { $PasswordFile = $File; $Load = $True }
    Else { $PasswordFile = ('C:\EOH_RT\Scripts\Config-' + $CommonName + '-Config.PSF') }
    If ($Create -eq $True) {
        If (Test-Path $PasswordFile) { Remove-Item $PasswordFile }
        $SavedCredentials = Get-Credential
        Out-File -FilePath $PasswordFile -Encoding ascii -Force -NoClobber -InputObject $SavedCredentials.UserName
        Out-File -FilePath $PasswordFile -Encoding ascii -Append -NoClobber -InputObject (ConvertFrom-SecureString $SavedCredentials.Password)
    }
    If ($Load -eq $True) {
        ...
        $PasswordFileContents = Get-Content $PasswordFile
        $LoadedPassword = New-Object -TypeName PSCredential($PasswordFileContents[0], (ConvertTo-SecureString $PasswordFileContents[1]))
    }
...
Function Get-HyperVVMS {
    ...
    Else {
        $VMs = Get-WmiObject -Query "Select * from MSVM_ComputerSystem Where Caption = 'Virtual Machine'" -Namespace 'root\virtualization'
        ForEach ($VM in $VMs) {
...
Write-Host ("Loading Modules - ") -ForegroundColor White -NoNewLine
    Switch ($ReportingEnvironment.Platform) {
        "Vmware"           { 
            If ($PasswordFile -ne '') { $Credentials = Setup-Credentials -File $PasswordFile -CommonName $ReportingEnvironment.CommonName }
            Else { $Credentials = Setup-Credentials -Load -CommonName $ReportingEnvironment.CommonName}
