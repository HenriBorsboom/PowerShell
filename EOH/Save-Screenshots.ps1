...
$WorkingDirectory = 'C:\Users\henri.borsboom\Pictures\Screenshots'
...
Write-Color -Text 'Collecting the dates - ' -ForegroundColor White -NoNewLine
    [String[]] $UniqueFolders = (($ScreenShots.CreationTime).Date.ToShortDateString()).Replace('/', '-') | select -Unique
...
For ($SC = 0; $SC -lt $ScreenShots.Count; $SC ++) {
    ...
    If (Test-Path ($WorkingDirectory + '\' + (($ScreenShots[$SC].CreationTime).Date.ToShortDateString()).Replace('/', '-'))) {
        Move-Item $ScreenShots[$SC].FullName -Destination ($WorkingDirectory + '\' + (($ScreenShots[$SC].CreationTime).Date.ToShortDateString()).Replace('/', '-'))
        Write-Color -Complete
    }
    Else {
...
#(($ScreenShots[$SC].CreationTime).Date.ToShortDateString()).Replace('/', '-') | select -Unique