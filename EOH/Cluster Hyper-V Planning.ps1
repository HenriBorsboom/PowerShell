...
Function VM-Storage {
    Param (
        ...
        [Object[]] $ClusterVMs, `
        [Parameter(Mandatory=$True)]
        [Object[]] $ClusterHostStorage)
    
    $ReturnDetails = @()
    For ($x = 0; $x -lt $ClusterHostStorage.Count; $x ++) {
        $RemotePath = $ClusterHostStorage[$x].Path -replace('C:', ('\\' + $ClusterHosts[0].NodeName + '\c$'))
        Write-Color -IndexCounter $x -TotalCounter $ClusterHostStorage.Count -Text 'Getting VMs on ', $RemotePath, ' - ' -ForegroundColor $MyColors.Text, $MyColors.Value, $MyColors.Text -NoNewline
        $CSVVMs = Get-ChildItem -Path $RemotePath -Directory
        Write-Color -Text $CSVVMs.Count, ' Found' -ForegroundColor $MyColors.Value, $MyColors.Value
        For ($y = 0; $y -lt $CSVVMs.Count; $y ++) {
            Write-Color -IndexCounter $y -TotalCounter $CSVVMs.Count -Text 'Gathering VM Details for ', $CSVVMs[$y].BaseName, ' - ' -ForegroundColor $MyColors.Text, $MyColors.Value, $MyColors.Text -NoNewLine
            If ($ClusterVMs.VMName.Contains($CSVVMs[$y].BaseName)) {
                $i = $ClusterVMs.VMName.IndexOf(($CSVVMs[$y].BaseName))
                $ReturnDetails += (New-Object -TypeName PSObject -Property @{
                    Node = $ClusterVMs[$i].Node.Name
                    VMName = $ClusterVMs[$i].VMName.Name
                    CPU = $ClusterVMs[$i].CPU
                    RAM = $ClusterVMs[$i].RAM
                    TotalFileSize = $ClusterVMs[$i].TotalFileSize
                    TotalVHDSize = $ClusterVMs[$i].TotalVHDSize
                    Volume = $ClusterHostStorage[$x].Path
                    VolumeSize = $ClusterHostStorage[$x].Size
                })
                Write-Color -Complete
            }
            Else {
                Write-Color -Text 'Not in Cluster' -ForegroundColor $MyColors.Error
            }
        }
    }
    $UniqueVolumes = $ReturnDetails | Select -Unique Volume
    $VolumeUsages = @()
    For ($b = 0; $b -lt $UniqueVolumes.Count; $b ++) {
        $VolumeSize = ($ReturnDetails | Where-Object {$_.Volume -eq $UniqueVolumes[$b].Volume}).VolumeSize[0]
        $TotalThin = ($ReturnDetails | Where-Object {$_.Volume -eq $UniqueVolumes[$b].Volume} | Measure-Object -Sum TotalFileSize).Sum
        $TotalThick = ($ReturnDetails | Where-Object {$_.Volume -eq $UniqueVolumes[$b].Volume} | Measure-Object -Sum TotalVHDSize).Sum
        $OverProvisioned = [Math]::Round($TotalThick / $VolumeSize * 100, 2)
        $VolumeUsages += ,(New-Object -TypeName PSObject -Property @{
            Volume = $UniqueVolumes[$b].Volume;
            VolumeSize = $VolumeSize
            TotalThin = $TotalThin
            TotalThick = $TotalThick
...
#$Cluster = 'SLBCLSTST200'
$ClusterVMs         = VM-Resources -Cluster $Cluster
$ClusterHosts       = Cluster-Resources -Cluster $Cluster                
$ClusterHostStorage = Get-CSVDetails -Cluster $Cluster
$VolumeUsages       = VM-Storage -Cluster $Cluster -ClusterVMs $ClusterVMs -ClusterHostStorage $ClusterHostStorage
#Reporting
...
$ClusterVMs | Select Node, VMName, CPU, RAM, TotalFileSize, TotalVHDSize | Format-Table -AutoSize
Write-Host ""
...
Write-Host "----------------------------" -ForegroundColor $MyColors.Warning
Write-Host ""
#endregion
#region Host
Write-Color -Text '------ ', 'Host Details' -ForegroundColor $MyColors.Warning, $MyColors.Value
$ClusterHosts | Select NodeName, CPU, RAM | Format-Table -AutoSize
...
$ClusterHostStorage | Select Name, Path, Size, FreeSpace, UsedSpace, PercentFree | Format-Table -AutoSize
...
#endregion
#region Volume Usage
Write-Color -Text '------ ', 'Storage Volume' -ForegroundColor $MyColors.Warning, $MyColors.Value
$VolumeUsages | Select Volume, VolumeSize, TotalThin, TotalThick, OverProvisioned  | Format-Table -AutoSize
Write-Host "----------------------------" -ForegroundColor $MyColors.Warning
#endregion