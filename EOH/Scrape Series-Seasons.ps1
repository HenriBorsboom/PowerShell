Function Rename-Series {
    ...
    If ($Season -lt 10) {
        $SourceFileLocation = ('\\192.168.1.103\Downloads\Series\' + $Series + '\Season 0' + $Season)
    }
    Else {
        $SourceFileLocation = ('\\192.168.1.103\Downloads\Series\' + $Series + '\Season ' + $Season)
    }
    If ($TTID -eq '') {
...
For ($i = 10; $i -lt 15; $i ++) {
    Write-Color -IndexCounter ($i - 1) -TotalCounter 14 -Text "Renaming Season ", $i -ForegroundColor White, Yellow
    Rename-Series -Season $i
}