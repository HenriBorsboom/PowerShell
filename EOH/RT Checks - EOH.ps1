Param (
    ...
    [Object] $vCenter)# = (New-Object -TypeName PSObject -Property @{ IPAddress = '10.1.5.254'   ; CommonName = 'EOH Midrand Waterfall'; Username = 'root'  ; Password = 'Passw00rd' ; Parameter = ''}))
...
$NonCriticalImage = Generate-HTMLImage -ImagePath 'C:\temp\Scripts\Non-Critical.png'
$WarningImage     = Generate-HTMLImage -ImagePath 'C:\temp\Scripts\Warning.png'
$CriticalImage    = Generate-HTMLImage -ImagePath 'C:\temp\Scripts\Critical.png'
#endregion
...
Function Get-vCenterAlarms {
    ...
    ForEach ($ActiveAlarm in $ActiveAlarms) {
        ...
        }) | Select AlarmName, OverallStatus, Time, HealthIcon
    }
...
Function Get-vCenterDatastores {
    ...
    ForEach ($Datastore in $Datastores) {
        ...
        $ReportDatastores += (New-Object -TypeName PSObject -Property @{
            ...
            DataStoreHealthIcon = $DatastoreHealth
        }) | Select Name, FreeSpaceMB, CapacityMB, FreePerc, DataStoreHealthIcon
    }
...
Function Process-vCenterAlarms {
    ...
    If ($ReportAlarms.Count -gt 0) {
        ...
        for ($AlarmsHTMLIndex = 1; $AlarmsHTMLIndex -le $AlarmsHTML.table.tr.count - 1; $AlarmsHTMLIndex++) {
            if ($AlarmsHTML.table.tr[$AlarmsHTMLIndex].td[3] -eq "[CriticalImage]") {
                $class = $AlarmsHTML.CreateAttribute("class")
                $class.value = 'alert'
                $AlarmsHTML.table.tr[$AlarmsHTMLIndex].attributes.append($class) | out-null
            }
            if ($AlarmsHTML.table.tr[$AlarmsHTMLIndex].td[3] -eq "[WarningImage]") {
                $class = $AlarmsHTML.CreateAttribute("class")
                $class.value = 'warning'
                $AlarmsHTML.table.tr[$AlarmsHTMLIndex].attributes.append($class) | out-null
            }
