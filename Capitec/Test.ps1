...
$ServerListFile = 'C:\iOCO Tools\Scripts\AllServerList.txt'
...
$AVGProcResults = If ($AVGProc -ge 90) {
   ...
   Else { "<b><u><p style=color:#00e600>GOOD AVG CPU: </p></b></u>"
   "$AVGproc%"
        }
...
$MemoryResult = If ($($OS.MemoryUsage) -ge 90) {
   ...
   Else { "<b><u><p style=color:#00e600>GOOD MEMORY: </p></b></u>"
   "$($OS.MemoryUsage)%"
        }
...
else {"~Green" + "{0:n2}"-f ($_.freespace/$_.size*100)}}} | ConvertTo-Html -Fragment
...
$result += [PSCustomObject] @{ 
    ...
    $Outputreport = "<HTML><TITLE> PG Group - Server Health Report $CurrentTime</TITLE>
                     ...
                     <H2> PG Group - Server Health Report $CurrentTime</H2></font>
...
$Outputreport | out-file "C:\Temp\Reports\All Servers\SNDDOMAIN_All Server Health Report $Savetime.html"