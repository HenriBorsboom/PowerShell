...
$DHCPDNSSettings = {
    ...
    $OutFile = ('C:\Temp\Henri\DNSChecks-DHCP\' + $Server + '_' + (Get-Date).ToString('yyyy-MM-dd HH_mm_ss') + '.txt')
    $CSVOutFile = ('C:\Temp\Henri\DNSChecks-DHCP\' + $Server + '_' + (Get-Date).ToString('yyyy-MM-dd HH_mm_ss') + '.csv')
    ...
    Try {
        ...
        $Result = Invoke-Command -ComputerName $server -Credential $Credential -ScriptBlock {
            ...
            Return $DHCPNameServer, $NameServer, $DHCPServerPublished, $DHCPInUse
        }
    ...
    $Details | Select-Object Server, DHCPNameServer, NameServer, DHCPServerPublished, DHCPInUse, Result | Export-CSV $CSVOutFile -Delimiter ',' -NoTypeInformation
}
...
$SBArgs = @($Credential)
Start-Jobs -ScriptBlock $DHCPDNSSettings -ScriptBlockArguments $SBArgs -Targets $Servers.Name -PassTargetToScriptBlock Both -MaximumJobs 10
