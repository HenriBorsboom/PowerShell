    ...
    For ($i = $ResumeI; $i -lt $Reports.Count; $i ++) {
        ...
        For ($x = $ResumeX; $x -lt $RestoreFiles.Count; $x ++) {
            ...
            If ($CopyFlag -eq $True) {
                ...
                If ($psISE -ne $null) {
                    Start-BitsTransfer -Source $RestoreFile -Destination ($RestoreFile.Replace('Y:\', $TargetRestoreFolder)) -Description ($RestoreFile.Replace('Y:\', $TargetRestoreFolder)) -DisplayName $RestoreFile
                }
                Else {
                    Copy-Item -LiteralPath $RestoreFile -Destination ($RestoreFile.Replace('Y:\', $TargetRestoreFolder)) -Force
                }
            ...
            ElseIf ((Get-ChildItem -LiteralPath $RestoreFile.Replace('Y:\', 'E:\')).Attributes -like '*l*') {
                If ((Get-ChildItem -LiteralPath $RestoreFile).Length -eq (Get-ChildItem -LiteralPath $RestoreFile.Replace('Y:\', 'E:\')).Length) {
                    ...
                    Catch {
                        ...
                        New-ACL -Path ($RestoreFile.Replace('Y:\', $TargetRestoreFolder)) -ACL $ACL
                        Remove-Item -LiteralPath ($RestoreFile.Replace('Y:\', $TargetRestoreFolder))
                    }
                    Finally {
                        If ($psISE -ne $null) {
