...
Function Set-HardenServer {
    $result = HardenServer -scorch $scorch -node $node01 -SDLC $SDLC -Location $Location -ClientSeceret $ClientSecret
    if ($result -ne 0) {
    ...
    $result = HardenServer -scorch $scorch -node $node02 -SDLC $SDLC -Location $Location -ClientSeceret $ClientSecret
    If ($result -ne 0) {
...
Try {
    If ($VCO1 -ne $null -or $VCO1 -ne "") {
        ValidateInputs -node01 $node01 -node02 $node02 -CNO $CNO -VCO $VCO -VCO1 $VCO1 -SDLC $SDLC -Location $Location -ClientSeceret $ClientSecret -ErrorAction stop
    }
    Else {
        ValidateInputs -node01 $node01 -node02 $node02 -CNO $CNO -VCO $VCO -SDLC $SDLC -Location $Location -ClientSeceret $ClientSecret -ErrorAction stop
    }
}
Catch{
    $error[0].Exception
