...
Function Set-Errors {
        ...
        Catch {
                Write-Log -LogFile $MainLogFile -Level Error -Component $MyInvocation.MyCommand.Name -Message ($_)
        }
} 
Function Send-Email {
        Param (
...
Function New-EventSummary {
        ...
        $EventFiles = Get-ChildItem -Path ($ReportFolder + ($StartTime.ToString("yyyy-MM-dd"))) -Recurse | Where-Object {$_.name -like '*Application Errors*' -or $_.Name -like '*Application Warnings*' -or $_.Name -like '*Security Warnings*' -or $_.Name -like '*System Warnings*' -or $_.Name -like '*Security Errors*'}
        $EventFiles = $EventFiles | Sort-Object BaseName
        $ErrorAmount = 0
        ForEach ($EventFile in $EventFiles) {
            If (!($EventFile.Name -like '*0 errors*')) {
                $ErrorAmount ++
                Switch ($DebugState) {
                    $True {
