...
Param (
    ...
    [String] $DomainFQDN = 'ansible.local',                                                                       # Fully qualified domain name where root records are stored
    [Parameter(Mandatory=$False, Position=4)]
    [String] $DnsZoneJumprootFinSky = 'bank.local',                                                            # DNS Sub domain where records are stored
    [Parameter(Mandatory=$False, Position=5)]
    [String] $DnsZoneJump = 'bank',                                                                      # DNS zone where records are kept
    [Parameter(Mandatory=$False, Position=6)]
    [String] $DnsZoneLinuxJump = 'bank',                                                                 # DNS zone where linux records are kept
    [Parameter(Mandatory=$False, Position=7)][ValidateScript({ Test-Path $_ -PathType Leaf })]  # Test if path to CSV file is valid
    [String] $RecordsFilePath  = "C:\Temp\records.csv",                                        # CSV file of records where permissions needs to be updated
