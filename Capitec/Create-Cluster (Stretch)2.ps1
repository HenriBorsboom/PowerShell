...
^CN=(.+?)(?:(?<!\\),|$)
"@
        $dn =  Get-ADComputer $CNO -Server CBDC004 #$LocalDC
        $OU = $($dn.DistinguishedName) -replace ($pattrn)
        #$OU
        New-ADComputer -Name $VCO -Description "Failover cluster virtual network name account" -Path $OU -Credential $vmmaction -Server CBDC004 #$LocalDC
...
Function SetVCOACL {
    ...
    $VCO = Get-ADComputer $VCO -Credential $vmmaction -Server CBDC004 #$LocalDC 
    write-output $VCO
    $CNO = Get-ADComputer $CNO -Credential $vmmaction -Server CBDC004 #$LocalDC 
    write-output $CNO
