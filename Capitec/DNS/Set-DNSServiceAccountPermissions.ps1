...
Param (
    ...
    [String] $ServiceAccountName,                                                               # AD Service account to create
    [Parameter(Mandatory=$True, Position=2)]
    [String] $ServiceAccountOU,                                                                 # AD OU where the service account should be created
    [Parameter(Mandatory=$True, Position=3)]
    [String] $DomainFQDN,                                                                       # Fully qualified domain name where root records are stored
    [Parameter(Mandatory=$True, Position=4)]
    [String] $DnsZoneJumprootFinSky,                                                            # DNS Sub domain where records are stored
    [Parameter(Mandatory=$True, Position=5)]
    [String] $DnsZoneJump,                                                                      # DNS zone where records are kept
    [Parameter(Mandatory=$True, Position=6)]
    [String] $DnsZoneLinuxJump,                                                                 # DNS zone where linux records are kept
    [Parameter(Mandatory=$False, Position=7)][ValidateScript({ Test-Path $_ -PathType Leaf })]  # Test if path to CSV file is valid
    [String] $RecordsFilePath  = "C:\Local\records.csv",                                        # CSV file of records where permissions needs to be updated
    [Parameter(Mandatory=$False, Position=8)]
    [Switch] $EnableLogging = $false                                                            # Enable script logging
)
...
If (-not $account) {                                                                             # Check if the account exists
    ...
    $Password = ConvertTo-SecureString (New-Guid).ToString() -AsPlainText -Force                 # Create a new random password for the account
