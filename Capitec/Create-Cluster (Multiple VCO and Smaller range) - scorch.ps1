...
Function CheckGroupMembership {
    ...
    Switch ($Location) {
        ...
        "BFTC" {$LocalDC = "CBDC003.capitecbank.fin.sky"}
    }
...
function RetreiveIPAMClusterIP
{
        ...
        return $FreeIP} -ArgumentList $IPAddress,$CNO, $Start, $End -ErrorAction stop
        return $FreeIP
...
{
    param($node)
    start-job -name "Restarting node $node" -ScriptBlock {param($node) Start-Sleep -seconds 10; Restart-Computer -ComputerName $node -Force} -ArgumentList $node
    ...
    param([Parameter(Mandatory=$true)]
          ...
          Try{
            ...
            switch ($Location) 
            {
                "BLV" {$LocalDC = "CBDC01.capitecbank.fin.sky"}
                "STB" {$LocalDC = "CBDC03.capitecbank.fin.sky"}
                "NP" {$LocalDC = "CBBFTCDC001.capitecbank.fin.sky"}
                "PRD" {$LocalDC = "CBDC002.capitecbank.fin.sky"}
                "DR" {$LocalDC = "CBDC003.capitecbank.fin.sky"}
...
$Start = read-host -Prompt "Please provide the Start IP of the subnet (Leave blank if /24 CIDR range)"
$End = read-host -Prompt "Please provide the End IP of the subnet (Leave blank if /24 CIDR range)"
