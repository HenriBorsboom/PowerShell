...
$EC2DNSHostSettings = {
    ...
    $OutFile = ('C:\Temp\Henri\EC2DNSHostChecks\' + $Server + '_' + (Get-Date).ToString('yyyy-MM-dd HH_mm_ss') + '.txt')
    $CSVOutFile = ('C:\Temp\Henri\EC2DNSHostChecks\' + $Server + '_' + (Get-Date).ToString('yyyy-MM-dd HH_mm_ss') + '.csv')
...
$SBArgs = @($Credential)
Start-Jobs -ScriptBlock $EC2DNSHostSettings -ScriptBlockArguments $SBArgs -Targets $Servers -PassTargetToScriptBlock Both -MaximumJobs 10
