...
Function RetreiveIPAMClusterIP {
    ...
    $FreeIP = Invoke-Command -ComputerName CBSTBIPAM01 -Credential $Scorch -ArgumentList $IPAddress,$CNO, $Start, $End -ScriptBlock {
        ...
        If (($null -eq $Start -or $Start -eq '') -and ($null -eq $end -or $End -eq '')) {
...
Function InstallFailoverCluster {
    param($node)
    invoke-command -computername $node  -Credential $vmmaction -scriptblock {
...
Function RestartServer {
    ...
    While (!(invoke-Command -ComputerName $node  -Credential $vmmaction -ScriptBlock {hostname})) {
...
Function Test-WMIConnectivity {
    Try {
        Get-WmiObject -Class Win32_OperatingSystem -ComputerName $node01 -ErrorAction stop -Credential $vmmaction
    ...
    Try {
        Get-WmiObject -Class Win32_Operatingsystem -ComputerName $node02 -ErrorAction stop -Credential $vmmaction
...
Function Set-HardenServer {
    $result = HardenServer -scorch $scorch -node $node01 -SDLC $SDLC -Location $Location -ClientSecret $ClientSecret
    if ($result -ne 0) {
    ...
    $result = HardenServer -scorch $scorch -node $node02 -SDLC $SDLC -Location $Location -ClientSecret $ClientSecret
    If ($result -ne 0) {
...
Function New-Cluster {
    ...
    While (((invoke-command -ComputerName $node01 -ScriptBlock {test-path c:\temp\clusteout.txt} -Credential $vmmaction) -eq $false) -and ((invoke-command -ComputerName $node01 -ScriptBlock {(get-content c:\temp\cluserout.txt).length -eq 0} -Credential $vmmaction))) {
        ...
        invoke-command -computername $node01 -scriptblock {Set-ItemProperty -Path HKLM:SYSTEM\CurrentControlSet\Control\Lsa -Name disabledomaincreds -Value 0 -Force} -Credential $vmmaction
        invoke-command -computername $node01 -scriptblock {start-scheduledtask ClusterCreate} -Credential $vmmaction
...
Function FullRun {
    ...
    If ($node01 -eq '' -or $null -eq $node01) {
        ...
        $ClientSecret = Read-Host -Prompt "Please enter the production clientID (SecurityServices) password to retrieve token from IDP"
        ...
        Try {
            If ($null -eq $VCO1 -or $VCO1 -ne "") {
                ValidateInputs -node01 $node01 -node02 $node02 -CNO $CNO -VCO $VCO -VCO1 $VCO1 -SDLC $SDLC -Location $Location -ClientSecret $ClientSecret -ErrorAction stop
            }
            Else {
                ValidateInputs -node01 $node01 -node02 $node02 -CNO $CNO -VCO $VCO -SDLC $SDLC -Location $Location -ClientSecret $ClientSecret -ErrorAction stop
            }
        }
        Catch{
            $error[0].Exception
