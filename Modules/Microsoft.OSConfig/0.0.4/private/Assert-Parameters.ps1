...
function Assert-Parameters([CmdletType] $CmdletType, [String] $Scenario, [String] $Version, [String] $Setting) {
    switch ($CmdletType) {
        ScenarioDefinition {
            if ($Scenario -and
                (-not (Test-Path -Path "$($Script:Constants.RegistryKeyPath.ScenarioSchema)\1.0\$Scenario")) -and
                (-not (Test-Path -Path "$PSScriptRoot/../resources/$Scenario"))) {
                throw [InvalidScenarioException]::new(($Strings.ErrorNotFoundScenario -f $Scenario))
            }

            if ($Version -and
                (-not (Test-Path -Path "$($Script:Constants.RegistryKeyPath.ScenarioSchema)\1.0\$Scenario\$Version")) -and
                (-not (Test-Path -Path "$PSScriptRoot/../resources/$Scenario/$Version"))) {
                throw [InvalidVersionException]::new(($Strings.ErrorNotFoundVersion -f $Version))
            }
        }
        DesiredConfiguration {
            if ((-not $Scenario) -or
                (-not (Test-Path -Path "$($Script:Constants.RegistryKeyPath.ScenarioSchema)\1.0\$Scenario"))) {
                throw [InvalidScenarioException]::new(($Strings.ErrorNotFoundScenario -f $Scenario))
            }

            if ((-not $Version) -or
                (-not (Test-Path -Path "$($Script:Constants.RegistryKeyPath.ScenarioSchema)\1.0\$Scenario\$Version"))) {
                throw [InvalidVersionException]::new(($Strings.ErrorNotFoundVersion -f $Version))
            }

            if ($Setting -and (-not (Test-Path -Path "$($Script:Constants.RegistryKeyPath.ScenarioSchema)\1.0\$Scenario\$Version\Settings\$Setting"))) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAdosuJ07DMnTlF
# RueyxJEyUZ6qIrfYSoaRiFw+HSVQjaCCDXYwggX0MIID3KADAgECAhMzAAADrzBA
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIOy4rAnC+J9pyhxQ5r8t0tVs
# goxnTpsAsHSfYxCsBrLKMEIGCisGAQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEB
# BQAEggEAbPNi3jWPN5LX1bEkWHN55JFTNzaRtgrnT7iwjJ47vUB8+HyyL48wVAmj
# w7Nfowj4t2uhbfnArBfTvxPH8smvbvPXhWQLcrwRUPTb1kvHFm3d0BZOdhv6mSfx
# Kh889VGnyhp/UiWtZamEWpbgQA/26wFM/FFLHXPcfczXtugVfldQaznOa1xlMTys
# Y+X3eof0DgifIou/vXbHL4X6FnTNe1BINoNIi2bHp34WEMJEHt5kHLfKFzfpC+d6
# vxkq7gB9Q5XLDw5WCkYN45x4ShgKwPPMbdG7J4eKrzYTGeiZBY7gcZhaRb17VDuS
# Q134SUSJef1SaJciSZg5Z7/BS6AIhaGCFyowghcmBgorBgEEAYI3AwMBMYIXFjCC
# FxIGCSqGSIb3DQEHAqCCFwMwghb/AgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFZBgsq
# hkiG9w0BCRABBKCCAUgEggFEMIIBQAIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFl
# AwQCAQUABCCCNl2EIBO8hF/t+uGUaQ7VXgWqPLtFW7trXdYf5MZR5wIGZdY1qYb5
# GBMyMDI0MDMxMjE5MDE1Ny4xODdaMASAAgH0oIHYpIHVMIHSMQswCQYDVQQGEwJV
...
# MSIEIN50GK6/lqnih8p+my4vckL5t14DU7qfRMI2qbF+4BU6MIH6BgsqhkiG9w0B
...
# CzANBgkqhkiG9w0BAQsFAASCAgCAkk3+QlccDe5wFhnSBztkc8dokwVFfTV47aa3
# 0vXCLv9xbegZAEufeVvoxGR5YQNVTt3FwFsPko81e8q17wIKJixqEsPZcDCPQvQ+
# 1xGeDWkKSxJVZVBroNoVjMc9W6LB3zN088FauaaSl21/CZjVb8I6cZxB+Ker1hpG
# KeEDc/aWBPMriFhv9hhzHWtPsCi7ETIt99xKNLvjqkMfanhnSeVEjOAm+PDGo1RQ
# 4z1XV7K+sTKjKGkcO6jYoR2731+KbNXQa3xXsXKnze43UzWJZPTl7tzm/ao5r8hu
# 46WtrnibCmhBOKZcsBBv3gAPdd5eVcmzEptKKksSpHMcAH6J4X/YkW7wtU7HX+61
# j0MM8yT6Z3QpqqFbp5OInW4QpRY24XRk9itg8rVYhDO2PMH2PNlu6UskxKLhWikj
# rlHDYDSoOuXGYwGGKEYXpKb1iC81HtXew3lE99LdJJvIMucSiaJWWv54BlXKlQ1g
# zNpKiq5+GplCK/Ud0l+ZEZWdGJ6wU6s/+ZjolGcEYcmtVVMtx2lRcCuhRV2DaELb
# utpxuqzlBR2bQZHPKCRZ/DH9lhYvYyYue8Vf+6VoI4Iz1znKC/aUlqHeZ1d/Fc/9
# FgTHFIBdqV8B/Y/zl8kAZTucvh1mSyehx++zMDHYzkUsdCl3WbjQRt3v0Wvlxsej
# 0RM4XQ==
