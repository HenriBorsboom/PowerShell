...
Write-Color -Text "Setting Global Variables - " -ForegroundColor White -NoNewLine
    ...
    If (Test-Path C:\Temp\adroles.ps1) { Remove-Item C:\Temp\adroles.ps1 }
    $DomainScript = @(
        '$ADRoles = @()'
        ...
        'Install-ADDSForest `'
            ...
            '-Force:$true'
            'Restart-Computer -Force') | Out-File C:\Temp\adroles.ps1 -Encoding ascii -Force -NoClobber
...
Write-Color -Text "Logging into Azure - " -ForegroundColor White -NoNewLine
    Add-AzureRmAccount -Credential $Creds | Out-Null
Write-Color -Text "Complete" -ForegroundColor Green
#endregion
...
Write-Color -Text "Creating Scripts Container - " -ForegroundColor White -NoNewLine
    ...
    Set-AzureStorageBlobContent -Container 'scripts' -File C:\temp\adroles.ps1 | Out-Null
Write-Color -Text "Complete" -ForegroundColor Green
#endregion
...
#endregion
#region Provision VM
Write-Color -Text "Provision VM - " -ForegroundColor White -NoNewLine
    New-AzureRmVM -ResourceGroupName $rgName -Location $locName -VM $VMConfig[0] | Out-Null
Write-Color -Text "Complete" -ForegroundColor Green
#endregion
#region VM Extension
Write-Color -Text "VM Extension - " -ForegroundColor White -NoNewLine
    Set-AzureRmVMCustomScriptExtension -Location $locName -Name "ADRolesTest01" -VMName $VMRole[0] -ResourceGroupName $rgName -FileName "adroles.ps1" -ContainerName "scripts" -StorageAccountName $stName -Run '.\adroles.ps1' | Out-Null
Write-Color -Text "Complete" -ForegroundColor Green
#endregion