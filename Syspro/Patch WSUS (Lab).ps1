...
$Creds = New-Object PSCredential("DOMAIN3\adminuser", (ConvertTo-SecureString -String "YourPasswordHere" -AsPlainText -Force))
$WSUSContentSharePath = "\\sysjhbsyscentre.sysproza.net\e$\WSUS\WsusContent"
Try {
    Get-PSDrive "P" -ErrorAction Stop | Out-Null
    Remove-PSDrive "P"
    New-PSDrive –Name “P” –PSProvider FileSystem –Root $WSUSContentSharePath -Credential $Creds | Out-Null
}
Catch {
    New-PSDrive –Name “P” –PSProvider FileSystem –Root $WSUSContentSharePath -Credential $Creds | Out-Null
}

$WSUSContentSharePath = "P:\"
$TemporaryDirectory = "D:\Temp"
$ScratchDirectory = "D:\Scratch"
$VHDFile = "D:\Template Patched.vhdx"
