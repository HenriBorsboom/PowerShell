...
$Server   = 'SYSPRO-DCVM'
$Message  = 'Investigating AD Replication Issues'
$Duration = '3600'

#Start-ServerScommaintenance -ServerName $Server -Message ($Message + " - " + $env:USERNAME) -MaintModeinMinutes $Duration
#Stop-ServerScommaintenance -ServerName $Server -Message ($Message + " - " + $env:USERNAME)
$Objects = Get-SCOMMaintenanceModeObjects
$AllProperties = @(
    'MonitoringObjectID'
    'StartTime'
    'EndTime'
...
$UniqueProperties = @(
    'Comments'
    'User')
#$Objects | Select MonitoringObjectId, Comments
#$Objects | Select $AllProperties | Format-Table -AutoSize
$Objects | Select -Unique $UniqueProperties | Format-Table -AutoSize

#$Exclusions1 = "Excluded from monitoring for performance monitoring"
#$Exclusions2 = "Server only on when requested by Technical Services"