...
If (Get-Module Hyper-V)
{
	...
	{
		try { $Workflow = [XML] (Get-Content ".\Workflow.xml") }
	...
	{
		try { $Variable = [XML] (Get-Content ".\Variable.xml") }
	...
	If ($Validate)
	{
		...
		For ($i = 1; $i -le $VMCount; $i++)
		{
			...
			If ($CreateVM)
			{
				...
				# Enables Specifies the virtual processor’s features are to be limited for compatibility when migrating the virtual machine to another host. (GENv1 /GENv2)
				$ProcessorCompatibility = Get-Value -Count $i -Value "ProcessorCompatibility"
				Write-Host "    Setting Processors Compatibility"
				if ($ProcessorCompatibility -eq $true)
				{
					Set-VMProcessor -VMName $VMName -ComputerName $VMHost -CompatibilityForMigrationEnabled $true
				}
				...
				# Set pagefile disk
				If (($PagefileDisk -ne $null) -and ($PagefileDisk -ne "") -and ($VMGen -ne "2"))
				{
					$DataDrive++
...
"@ | Out-File "$Drive`:\Temp\Setup.ps1" -Encoding ASCII
         If ($VMGen -eq "2") {
@"
             If ((Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")}).Count -eq 1) 
             { 
             ...
             ElseIf ((Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")}).Count -eq 2) 
             {
                `$DBDiskNum = 1
                `$LogDiskNum = 1
                `$DatabasePath = (Get-Partition -DiskNumber (Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")} | Sort-Object Number)[`$DBDiskNum].Number).DriveLetter + ":\Windows\NTDS"
                `$LogPath = (Get-Partition -DiskNumber (Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")} | Sort-Object Number)[`$LogDiskNum].Number).DriveLetter + ":\Windows\NTDS"
                `$SysvolPath = (Get-Partition -DiskNumber (Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")} | Sort-Object Number)[`$DBDiskNum].Number).DriveLetter + ":\Windows\SYSVOL"
             }
             ElseIf ((Get-Disk | Where-Object {(`$_.BusType -eq "SCSI") -or (`$_.BusType -eq "SAS")}).Count -gt 2) 
             {
...
@"
		...
		Write-Host "  [DEBUG] ADDSPath: `$ADDSPaths"
		Write-Host "  [DEBUG] DatabasePath :`$DatabasePath"
		Write-Host "  [DEBUG] LogPath :`$LogPath"
		Write-Host "  [DEBUG] SysVolPath: `$SysvolPath"
