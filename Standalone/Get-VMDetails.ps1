...
#region VHD
    ...
    ForEach ($VHD in $VM.HardDrives){
        ...
        $VHDDetails = Get-VHDDetails -VHDPath $VHD.Path -VMHost $VMHostName
        $y = 1
        ForEach ($VHDInfo in $VHDDetails){
            $VHDText = "VHD" + $x + "_VHD_Format" + $y; $Output | Add-Member -MemberType NoteProperty -Name $VHDText -Value $VHDInfo.VHDFormat
            $VHDText = "VHD" + $x + "_VHD_Type" + $y; $Output | Add-Member -MemberType NoteProperty -Name $VHDText -Value $VHDInfo.VHDType
            $VHDText = "VHD" + $x + "_VHD_FileSize" + $y; $Output | Add-Member -MemberType NoteProperty -Name $VHDText -Value $VHDInfo.FileSize
            $VHDText = "VHD" + $x + "_VHD_Size" + $y; $Output | Add-Member -MemberType NoteProperty -Name $VHDText -Value $VHDInfo.Size
            $y ++
        }
        $y = 1
        $x ++
...
Function Get-VHDDetails{
    Param([Parameter(Mandatory=$True,Position=1)]
            [String] $VHDPath, `
          [Parameter(Mandatory=$True,Position=2)]
            [String] $VMHost)

    $VHDDetails = Get-VHD -Path $VHDPath -ComputerName $VMHost
    
    $Output = New-Object PSObject
    $Output | Add-Member -MemberType NoteProperty -Name VHDFormat -Value $VHDDetails.VHDFormat
    $Output | Add-Member -MemberType NoteProperty -Name VHDType -Value $VHDDetails.VHDType
    $Output | Add-Member -MemberType NoteProperty -Name FileSize -Value $VHDDetails.FileSize
    $Output | Add-Member -MemberType NoteProperty -Name Size -Value $VHDDetails.Size
    Return $Output
}
...
Function Combine-Details{
    ...
    Write-Host "Gathering TXT files - " -NoNewline
        $TextFiles = Get-ChildItem ".\*.txt" | select Name
    Write-Host "Complete" -ForegroundColor Green
...
#{
    ...
    #Get-VMsPerHost -VMHosts $VMHosts
