...
#>
    Param(
        ...
        [Validatescript({
			If ($SCVirtualSwitchesSet -contains $_) {
                $true
            }
			Else {
                Throw $SCVirtualSwitchesSet
            }
		})]
        [String]$SCVirtualSwitch, `
        [Parameter(Mandatory=$True,Position=3)]
        [Validatescript({
			If ($SCIPPoolsSet -contains $_) {
                $true
            }
			Else {
                Throw $SCIPPoolsSet
        ...
        [Validatescript({
			If ($SCVMNetworkSet -contains $_) {
                $true
            }
			Else {
                Throw $SCVMNetworkSet
            }
		})]
        [String]$SCVMNetwork, `
    ...
    $VMNet = Get-SCVMNetwork -Name $SCVMNetwork

    Try {
    #region Obtain VM Details
        Write-Host "Obtaining VM Details - " -NoNewline
        Write-Host "$SCVM - " -ForegroundColor Cyan -NoNewline
...
#region Validation Sets
    ...
    $SCVirtualSwitchesCollection = Get-SCVirtualNetwork
    $SCVirtualSwitchesSet = $SCVirtualSwitchesCollection.Name
    #endregion
    #region IP Address Pools
    $SCIPPoolsCollection = Get-SCStaticIPAddressPool
    $SCIPPoolsSet = $SCIPPoolsCollection.Name
    #endregion
    #region VM Network
    $SCVMNetworkCollection = Get-SCVMNetwork
    $SCVMNetworkSet = $SCVMNetworkCollection.Name
    #endregion
#endregion
